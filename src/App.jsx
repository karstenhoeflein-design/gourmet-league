...
async function searchRestaurantsAI(q) {
  ...
  if (lb !== -1) { try { const sub = (s !== -1 ? clean.slice(s) : "[" + clean); const p = JSON.parse(sub.slice(0, sub.lastIndexOf("}") + 1) + "]"); if (Array.isArray(p) && p.length) return normalizeRestaurants(p); } catch {} }
  throw new Error("Keine gültigen Restaurantdaten erhalten");
}
